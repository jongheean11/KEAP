...
namespace Fizbin.Kinect.Gestures.Segments
{
    ...
    /// </summary>
    public class RightDownSegments1 : IRelativeGestureSegment
    {
        /// <summary>
        /// Checks the gesture.
        /// </summary>
        ...
        public Dictionary<GesturePartResult, float> CheckGesture(Skeleton skeleton,bool leftHandGrip,bool rightHandGrip)
        {
            ...
            if (skeleton.Joints[JointType.HandLeft].Position.Z < skeleton.Joints[JointType.ShoulderLeft].Position.Z - 0.2 && rightHandGrip == true)
            {
                    ...
                    )
                {
                    ...
                    //(XY평면에서의 각도)
                    if (Math.Atan2(Convert.ToDouble(skeleton.Joints[JointType.HandLeft].Position.X - skeleton.Joints[JointType.ShoulderLeft].Position.X),
                 Convert.ToDouble(skeleton.Joints[JointType.ShoulderLeft].Position.Y - skeleton.Joints[JointType.HandLeft].Position.Y)) < 0.98 &&
                 ...
                 Convert.ToDouble(skeleton.Joints[JointType.ShoulderLeft].Position.Y - skeleton.Joints[JointType.HandLeft].Position.Y)) > 0.58)
                    {
                        //ZY평면에서의 각도
                        
                        if (Math.Atan2(Convert.ToDouble(skeleton.Joints[JointType.ShoulderLeft].Position.Y - skeleton.Joints[JointType.HandLeft].Position.Y),
                        Convert.ToDouble(skeleton.Joints[JointType.ShoulderLeft].Position.Z - skeleton.Joints[JointType.HandLeft].Position.Z)) < 0.98 &&
                        Math.Atan2(Convert.ToDouble(skeleton.Joints[JointType.ShoulderLeft].Position.Y - skeleton.Joints[JointType.HandLeft].Position.Y),
